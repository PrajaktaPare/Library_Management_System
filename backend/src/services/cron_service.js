...
import EmailHelper from '../utils/email_helper.js';
...
class CronService {
  ...
  static async sendDueDateReminders() {
    try {
      const sql = `SELECT br.id, br.due_date, b.title, u.name, u.email
        FROM book_requests br JOIN books b ON br.book_id = b.id JOIN users u ON br.student_id = u.id
        WHERE br.request_status = 'issued' AND br.due_date = DATE_ADD(CURDATE(), INTERVAL 2 DAY)`;
  ...
  static async calculateOverdueFines() {
    try {
      const sql = `SELECT br.id, br.due_date, br.fine_amount, b.title, u.name, u.email
        FROM book_requests br JOIN books b ON br.book_id = b.id JOIN users u ON br.student_id = u.id
